...
public class GetDocsQueueDirectoryAndFilenameTest : IClassFixture<ConfigurationFixture>
{
    ...
    public GetDocsQueueDirectoryAndFilenameTest(ConfigurationFixture fixture)
    {
    ...
    [Fact]
    public async Task GetDocsQueueDirectoryAndFilename_ShouldReturn_RootFolderNotFound()
    {
        // Arrange
        ...
        // Act
        Result<FileInfoDto> result = await sut.GetDocsQueueDirectoryAndFilename(mockData.GetDocumentSubmitRequestDto(container.Pro.Value), mockData.GetSessionData().Value);
